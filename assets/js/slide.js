...
jQuery(window).load(function() {
	...
	loadSlides(['slide1', 'slide2'], i);

	/*
    var page_ids = ['slide1', 'slide2'];
    var page_map = {};
    var page_contents = [];
    var deferreds = [];
    var deferred = function(page_id) {
		console.log("call " + page_id);
		jQuery.when(
			jQuery.ajax({
				url: "/slides/" + page_id + ".html"
			}).done(function(data) {
				page_map[page_id] = data;
				page_contents.push(data);
				return data;
			})
		).then(function(data) {
    ...
    for (var v=0; v<page_ids.length; v++) {
		//deferreds.push(deferred(page_ids[v]));
		var page_id = page_ids[v];
		deferreds.push(
			jQuery.ajax({
				url: "/slides/" + page_id + ".html"
	...
	jQuery.when.apply(jQuery, deferreds).then(function(results) {
		console.log("deferred - done ");
		console.log(results);
		var objects=arguments;
		for (var v=0; v<page_ids.length; v++) {
			//page_contents.push(page_map[page_ids[v]]);
			console.log("==================deferred - done ");
			console.log(page_map[page_ids[v]]);
			console.log("==================deferred - done ");
		}
		i(page_contents);
	});
